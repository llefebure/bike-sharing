...
Measuring the success of these models is difficult because the error rate for each station will naturally be different because of different demand. Some stations have more demand and thus higher variance. I'll benchmark the model against weekday/hour averages.
...
My focus in this analysis was to leverage the assumptions of Poisson regression to make proabilistic predictions that have more practical use than point predictions. In theory, this is a really great approach, and my analysis above shows that it can produce very useful looking predictions. However, a prediction is only as good as its accuracy. This model seems to outperform group average predictions slightly. 

There also may be a growth compononent.

Features to include are holiday flags, special events flags, and more granular weather data (hourly and add precipitation). I could also add dow and bump up regularization.